...
def inspect(db_path: Path) -> dict:
    ...
    # 10) T4 工作节奏自审(v3.5: 2026-09-11 快照,沿用 0910 模式"自举入库 + plan 闭环"双 commit)
    #     v3.4 沿用:v3.4 = 0910 巡检第 1/3 项原文 + 节奏 4「B4 4 失效 + 无下次软截止」
    #     v3.5 升级:
    #       - 节奏 4 升级:B4 4 失效 → 5 失效,无下次软截止 → 无任何软截止(0910 23:59 隐性软截止线已过)
    #       - 新增节奏 5「B4 五失效 + 无任何软截止对 T4 工作流的影响」(0911 巡检第 1 项)
    #       - t4_workflow_stage 升级:5 例形成(0910 实际触发)+ 沿用 0909 模式「自举入库 + plan 闭环」双 commit 3 例形成(0906/0909/0910)
    #     0911 巡检第 1 项原文:"B4 失效第 11 日 + 0831 承诺 + 0907 + 0908 + 0909 + 0910 = 五失效待声明 + 0910 23:59 隐性软截止线已过 = 无任何软截止窗口"
    #     0911 巡检第 3 项原文:"T4 沿用 0902 模式「巡检后 plan 文档闭环」小节奏已 5 例形成(0902/0906/0908/0909/0910) + 沿用 0909 模式「自举入库 + plan 闭环」双 commit 3 例形成(0906/0909/0910) + T4 工作流进入「巡检后 plan 文档闭环小节奏」+「自举入库节奏恢复」双层节奏稳定阶段"
    #     0911 巡检第 1 项新增:"0911 23:59 是 T1 决策点 1 在「无下次软截止 + 隐性软截止线也过」状态下的「全无软截止」首日"
    #     T4 边界:这是 T4 自审段(看 T4 自己节奏),不替 T1 决策;与 §8 阻塞项(B1-B4)/ §9 决策点(D1-D5)三视角互补
    #     §8 看「阻塞项」/ §9 看「决策点」/ §10 看「T4 自己节奏」——三视角构成 T1 + T4 协作的全貌
    #     v3.5 新增:T4 工作流在"B4 5 失效 + 无任何软截止(0910 23:59 已过)"环境下仍稳定输出,定性升级为"硬截止全失效下 T4 工作流自主稳定"——T4 与 T1 解耦彻底
    report["t4_cadence"] = {
        "snapshot_date": "2026-09-11",
        # 节奏 1:"巡检后 plan 文档闭环"小节奏 5 例形成(0910 实际触发 + 0911 巡检后 T4 是否触发第 6 例待观察)
        "rhythm_plan_close": {
            "examples": [
                {
                    "date": "0902",
            ...
            "count": 5,
            "cadence": "巡检触发 + 24h 内 plan 文档闭环(非积压破局亦非自举入库,是稳定的 plan 文档清理节奏);0911 巡检后 T4 是否触发第 6 例(闭环 0911 plan 文档)待观察",
        },
        # 节奏 2:T4 即时闭环节奏小结(0911 巡检后 5 段观察,4 触发 1 未触发)
        "rhythm_t4_immediate": {
            "observations": [
            ...
            "summary": "五次确认「巡检触发」性质(4 触发 1 未触发);0909 模式升级「自举入库 + plan 闭环」双 commit,0910 沿用同样模式 → T4 工作流进入「自举 + plan 闭环」双层稳定节奏;0911 巡检后 T4 是否触发本周期(0911 v3.5 + 0911 plan 闭环)待观察",
        },
        # 节奏 3:0830 mavis daemon 单次漏跑正式降级(0908 降级 + 0911 沿用)
        "rhythm_0830_daemon": {
            "observation": "0830 单次漏跑后连续 11 日稳定(0831/0901/0902/0903/0904/0905/0906/0907/0908/0909/0910/0911 巡检正常跑出)",
            "verdict": "降级为单次偶发,0908 起转附录式 1 句记录,后续巡检不再重复根因排查建议",
            "rationale": "连续 11 日稳定 + 巡检 16 次累计 + 0830 是唯一漏跑,判定为单次偶发而非系统性问题",
        },
        # 节奏 4(v3.5 升级):B4 五失效 + 无任何软截止(0910 23:59 隐性软截止线已过)对 T4 工作流的影响
        "rhythm_b4_impasse": {
        ...
        # 节奏 5(v3.5 新增):B4 五失效 + 无任何软截止对 T4 工作流的「零缓冲」首日观察
        "rhythm_b5_impasse": {
        ...
        # T4 工作流新阶段定性(v3.5 升级)
        "t4_workflow_stage": "新阶段 = 「巡检后 plan 文档闭环小节奏」(5 例形成:0902/0906/0908/0909/0910) + 「自举入库 + plan 闭环」双 commit 模式(3 例形成:0906/0909/0910)双层稳定节奏;B4 5 失效 + 无任何软截止(0910 23:59 隐性软截止线已过)环境下 T4 工作流自主稳定,三阶段演化(有 soft deadline / 无下次软截止 / 无任何软截止)均不改变 T4 节奏",
    }
...
def render_md(r: dict) -> str:
    ...
    # T4 工作节奏自审(v3.5: 2026-09-11 快照,"巡检后 plan 文档闭环"小节奏 5 例形成 + T4 即时闭环 5 段观察 + 节奏 4 升级 + 节奏 5 新增)
    if r.get("t4_cadence"):
        c = r["t4_cadence"]
        ...
        # 节奏 1:巡检后 plan 文档闭环小节奏(5 例形成)
        rc = c["rhythm_plan_close"]
        lines.append("### 10.1 巡检后 plan 文档闭环小节奏(5 例形成)")
        lines.append("")
        ...
        # 节奏 2:T4 即时闭环节奏小结(4 触发 1 未触发,0911 巡检后 T4 是否触发待观察)
        ri = c["rhythm_t4_immediate"]
        lines.append("### 10.2 T4 即时闭环节奏小结(4 触发 1 未触发,0911 巡检后 T4 触发待观察)")
        lines.append("")
        ...
        lines.append("### 10.4 B4 五失效 + 无任何软截止对 T4 工作流的影响(v3.5 升级)")
        lines.append("")
        ...
        lines.append("### 10.5 B4 五失效 + 无任何软截止对 T4 工作流的「零缓冲」首日观察(v3.5 新增)")
        lines.append("")
