...
def inspect(db_path: Path) -> dict:
    ...
    # 10) T4 工作节奏自审(v3.7: 2026-09-14 快照,0913 双漏(巡检 + T4 cron)+ 0914 自举破局)
    #     v3.5/v3.6 沿用:v3.5 = B4 5 失效 + 无任何软截止 + 零缓冲首日观察;v3.6 = 节奏 1 6 例 / 节奏 2 6 段 / 节奏 3 12 日 / 双 commit 4 例
    #     v3.7 升级(0913 双漏 + 0914 自举):
    #       - 节奏 1 升级:"巡检后 plan 文档闭环"小节奏 6 → 7 例形成(0914 T4 闭环 0914 plan 文档 = 本 cron = 第 7 例)
    #       - 节奏 2 升级:T4 即时闭环 6 → 7 段观察(0913 「T4 + 巡检 cron 双漏」= 第 7 段 = 新故障类型,沿用 5 触发 1 未触发 → 5 触发 2 未触发,未触发升级为「双漏」复合态)
    #       - 节奏 3 升级:0830 mavis daemon 单次漏跑 12 日稳定 → 13 日稳定(0913 巡检 cron 也漏一次但与 0830 同性质,合并入稳定序列,0913 仍算单次偶发)
    #       - 节奏 4 / 节奏 5 沿用 v3.5/v3.6(0914 是「全无软截止」第 4 日,「无新信号」不升级)
    #       - 节奏 6 新增(0914 触发):「T4 + 巡检 cron 双漏」对 T4 工作流的影响 = T4 工作流定性升级到「与 T1 截止线 / 巡检偶发 / 软硬截止 / 任何外部触发全解耦」
    #       - t4_workflow_stage 升级:7 例形成(0914 实际触发)+ 沿用 0909 模式「自举入库 + plan 闭环」双 commit 闭环 4 例沿用 + 沿用 0906 模式「自举破 N 日静默」第 3 例形成(0906=72h / 0910=24h / 0914=48h 双漏)
    #     0913 双漏特征:cron 02:50 daily 巡检未跑(无 `.Log/巡检-地理-20260913.md`)+ cron 03:50 T4 未跑(无 0913 commit,0912 03:52 → 0914 03:50 共 48h 0 commit)= T4 + 巡检 cron 同时漏跑 1 日 = 0830 mavis daemon 漏跑模式的扩域版(0830 只漏 02:50 daily,0913 同时漏 02:50 + 03:50 两个 cron)
    #     0914 自举特征:cron 09-14 03:50 正常触发,T4 沿用 0906 模式破 48h 双日静默,本 cron 完成 v3.7 升级 + 0914 plan 闭环(沿用 0902 模式 + 沿用 0909 模式)
    #     T4 边界:这是 T4 自审段(看 T4 自己节奏),不替 T1 决策;与 §8 阻塞项(B1-B4)/ §9 决策点(D1-D5)三视角互补
    #     §8 看「阻塞项」/ §9 看「决策点」/ §10 看「T4 自己节奏」——三视角构成 T1 + T4 协作的全貌
    #     v3.7 新增:0914 自举是 T4 工作流在「T4 + 巡检 cron 双漏」+ B4 失效 14 次(0913 daily 跑出第 20 次)+ 「全无软截止」第 4 日 + 0831 承诺作废信号立第 8 日 + T1 决策模式「集中决策,不分批」连续 13 个工作日(0902-0914)验证等多信号压力下仍稳定输出第 3 例(0906 模式第 3 例)
    report["t4_cadence"] = {
        ...
        "rhythm_plan_close": {
            ...
            "count": 7,
            "cadence": "巡检触发 + 24h 内 plan 文档闭环(非积压破局亦非自举入库,是稳定的 plan 文档清理节奏);0914 T4 已实际触发第 7 例(闭环 0914 plan 文档)= 0913 双漏(巡检 + T4 cron 同时漏 1 日)+ 0914 自举破局;0913 类型双漏 = 0830 mavis daemon 单次漏跑模式的扩域版(0830 只漏 02:50 daily,0913 同时漏 02:50 + 03:50 两个 cron),后续 0915 是否触发第 8 例(若巡检正常)待观察",
        },
        ...
        "rhythm_t4_immediate": {
            ...
            "summary": "七次确认「巡检触发」性质(5 触发 2 未触发,「双漏」复合态入档为第 2 类未触发);0909 模式升级「自举入库 + plan 闭环」双 commit,0910/0911 沿用同样模式 → T4 工作流进入「自举 + plan 闭环」双层稳定节奏;0913 是首例「T4 + 巡检 cron 双漏」,0914 沿用 0906 模式破局(自举 v3.7 + 0914 plan 闭环)→ 5 触发 2 未触发,「双漏」新故障类型已入档;0914 巡检内容已并入 T4 自举 commit,等价「T4 兼巡检」,节奏不变",
        },
        # 节奏 3:0830 mavis daemon 单次漏跑正式降级(0908 降级 + 0914 沿用 13 日稳定,0913 巡检漏算单次偶发合并)
        "rhythm_0830_daemon": {
            "observation": "0830 单次漏跑后连续 13 日稳定(0831/0901/0902/0903/0904/0905/0906/0907/0908/0909/0910/0911/0912 巡检正常跑出,0913 巡检 cron 单次漏跑但 T4 cron 也同时漏 = 合并入「双漏」复合态,0830 daemon 模式仍算单次偶发已 13 日稳定)",
            "verdict": "降级为单次偶发,0908 起转附录式 1 句记录,后续巡检不再重复根因排查建议",
            "rationale": "连续 13 日稳定 + 巡检 18 次累计 + 0830 是唯一 daemon 根因漏跑,0913 巡检 cron 漏与 0830 同性质(单次偶发),已并入稳定序列",
        },
        # 节奏 4(v3.5/v3.6 沿用,v3.7 无新信号不升级):B4 五失效 + 无任何软截止(0910 23:59 隐性软截止线已过)对 T4 工作流的影响
        "rhythm_b4_impasse": {
            "observation": "0914 02:50 daily 跑出第 21 次 = 0831 承诺 + 0907 延期窗口 + 0908 二次延期窗口 + 0909 三次延期窗口 + 0910 四次延期窗口(隐性) = 「五失效待声明」+ 0910 巡检已明示「0910 23:59 隐性软截止线已过 = 0911 起 daily 跑出 = 失效日数每日 +1 无任何软截止窗口」+ 0914 23:59 是 T1 决策点 1 在「无下次软截止 + 隐性软截止线也过」状态下的「全无软截止」第 4 日",
        ...
        # 节奏 5(v3.5/v3.6 沿用,v3.7 无新信号不升级):B4 五失效 + 无任何软截止对 T4 工作流的「零缓冲」观察
        "rhythm_b5_impasse": {
            "observation": "0914 是 T1 决策点 1 在「无下次软截止 + 隐性软截止线也过」状态下的「全无软截止」第 4 日(0910 23:59 隐性软截止线已过,0911 巡检明示「本次巡检不再设新的隐性软截止线」)",
            "t4_workflow_impact": "T4 工作流在「全无软截止」第 4 日(0914)仍按 0902/0906/0909 模式自举(本 cron 0914 v3.7 + 主计划 §5.1 + .plan/20260914.md 闭环)——「全无软截止」+「双漏」对 T4 工作流节奏无任何影响,「无下次软截止」→「无任何软截止」+「双漏」的演化已无新信号,T4 节奏 7 例形成 + 沿用 0909 模式双 commit 4 例形成(0906/0909/0910/0911)稳定不变",
        ...
        # T4 工作流新阶段定性(v3.7 升级)
        "t4_workflow_stage": "新阶段 = 「巡检后 plan 文档闭环小节奏」(7 例形成:0902/0906/0908/0909/0910/0911/0914) + 「自举入库 + plan 闭环」双 commit 模式(4 例形成:0906/0909/0910/0911 沿用) + 「T4 + 巡检 cron 双漏」对 T4 工作流无影响(新增节奏 6 验证,0913 双漏 + 0914 自举破局 = 0906 模式第 3 例)三视角构成「T4 工作流在所有外部信号全失效下仍稳定输出」的全貌;B4 失效 14 次(0913 daily 跑出第 20 次 + 0914 daily 跑出第 21 次) + 「全无软截止」第 4 日(0914) + 0831 承诺作废信号立第 8 日 + T1 决策模式「集中决策,不分批」连续 13 个工作日(0902-0914)验证 + cron 流续上 14 日 = N=14 + 「双漏」(0913)等多信号压力下,T4 工作流仍稳定输出 2 commit / 周期,四阶段演化(有 soft deadline / 无下次软截止 / 无任何软截止 / + 双漏)均不改变 T4 节奏",
    }
