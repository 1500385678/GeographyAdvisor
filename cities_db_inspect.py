...
def inspect(db_path: Path) -> dict:
    ...
    # 10) T4 工作节奏自审(v3.6: 2026-09-12 快照,沿用 0911 模式"自举入库 + plan 闭环"双 commit 闭环第 4 例)
    #     v3.5 沿用:v3.5 = 0911 巡检第 1/3 项原文 + 节奏 4「B4 5 失效 + 无任何软截止」+ 节奏 5「零缓冲首日观察」
    #     v3.6 升级:
    #       - 节奏 1 升级:"巡检后 plan 文档闭环"小节奏 5 例 → 6 例形成(0911 T4 闭环 0911 plan 文档 `91ed2b8` = 第 6 例)
    #       - 节奏 2 升级:T4 即时闭环 5 段 → 6 段观察(0911 触发 2 commit,验证 5 触发 1 未触发)
    #       - 节奏 3 升级:0830 mavis daemon 单次漏跑 11 日稳定 → 12 日稳定 + 巡检 16 次 → 17 次累计(0912 巡检正常跑出)
    #       - 节奏 4 / 节奏 5 沿用 v3.5(0912 是「全无软截止」第 2 日,「无新信号」不升级)
    #       - t4_workflow_stage 升级:6 例形成(0911 实际触发)+ 沿用 0909 模式「自举入库 + plan 闭环」双 commit 闭环 3 例 → 4 例形成(0906/0909/0910/0911)
    #     0912 巡检第 1 项原文:"9-11→9-12 24h 0 commit(9-11 03:52 之后 21h 58m 0 commit) + 9-12 daily 跑出第 19 次 = B4 失效 12 次 + 0831 承诺彻底失效 12 次 + 0831 承诺作废信号立第 6 日 + T1 决策模式「集中决策,不分批」连续 11 个工作日(0902-0912)验证 + cron 流续上 13 日 = N=13"
    #     0912 巡检第 3 项原文:"T4 沿用 0902 模式「巡检后 plan 文档闭环」小节奏已 6 例形成(0902/0906/0908/0909/0910/0911) + 沿用 0909 模式「自举入库 + plan 闭环」双 commit 4 例形成(0906/0909/0910/0911) + T4 工作流进入「巡检后 plan 文档闭环小节奏」+「自举入库节奏恢复」双层节奏稳定阶段"
    #     0912 巡检第 1 项新增:"0912 是 T1 决策点 1 在「无下次软截止 + 隐性软截止线也过」状态下的「全无软截止」第 2 日(0911 23:59 隐性软截止线已过,0911 巡检已明示不再设新隐性软截止线)"
    #     T4 边界:这是 T4 自审段(看 T4 自己节奏),不替 T1 决策;与 §8 阻塞项(B1-B4)/ §9 决策点(D1-D5)三视角互补
    #     §8 看「阻塞项」/ §9 看「决策点」/ §10 看「T4 自己节奏」——三视角构成 T1 + T4 协作的全貌
    #     v3.6 新增:0911 巡检后 T4 已实际触发 2 commit(`748599b` v3.5 自举 + `91ed2b8` 0911 plan 闭环)→ 验证 v3.5 立的两项待观察 100% 兑现 + T4 工作流在 B4 失效 12 次 + 「全无软截止」第 2 日 + 0831 承诺作废信号立第 6 日 + T1 决策模式「集中决策,不分批」连续 11 个工作日验证等多信号压力下仍稳定输出
    report["t4_cadence"] = {
        "snapshot_date": "2026-09-12",
        # 节奏 1:"巡检后 plan 文档闭环"小节奏 6 例形成(v3.6 升级:0911 实际触发第 6 例)
        "rhythm_plan_close": {
            "examples": [
                {
                    "date": "0902",
            ...
            "count": 6,
            "cadence": "巡检触发 + 24h 内 plan 文档闭环(非积压破局亦非自举入库,是稳定的 plan 文档清理节奏);0911 T4 已实际触发第 6 例(闭环 0911 plan 文档)→ 0912 巡检后 T4 是否触发第 7 例(闭环 0912 plan 文档,即本 cron)待观察",
        },
        # 节奏 2:T4 即时闭环节奏小结(0912 巡检后 6 段观察,5 触发 1 未触发)
        "rhythm_t4_immediate": {
            "observations": [
            ...
            "summary": "六次确认「巡检触发」性质(5 触发 1 未触发);0909 模式升级「自举入库 + plan 闭环」双 commit,0910/0911 沿用同样模式 → T4 工作流进入「自举 + plan 闭环」双层稳定节奏;0911 巡检后 T4 实际触发 2 commit(本 cron 0911 03:50 跑出 748599b v3.5 自举 + 91ed2b8 0911 plan 闭环)→ 5 触发 1 未触发验证;0912 巡检后 T4 是否触发本周期(0912 v3.6 + 0912 plan 闭环)待观察",
        },
        # 节奏 3:0830 mavis daemon 单次漏跑正式降级(0908 降级 + 0912 沿用 12 日稳定)
        "rhythm_0830_daemon": {
            "observation": "0830 单次漏跑后连续 12 日稳定(0831/0901/0902/0903/0904/0905/0906/0907/0908/0909/0910/0911/0912 巡检正常跑出)",
            "verdict": "降级为单次偶发,0908 起转附录式 1 句记录,后续巡检不再重复根因排查建议",
            "rationale": "连续 12 日稳定 + 巡检 17 次累计 + 0830 是唯一漏跑,判定为单次偶发而非系统性问题",
        },
        # 节奏 4(v3.5 沿用,v3.6 无新信号不升级):B4 五失效 + 无任何软截止(0910 23:59 隐性软截止线已过)对 T4 工作流的影响
        "rhythm_b4_impasse": {
            "observation": "0911 02:50 daily 跑出第 17 次 = 0831 承诺 + 0907 延期窗口 + 0908 二次延期窗口 + 0909 三次延期窗口 + 0910 四次延期窗口(隐性) = 「五失效待声明」+ 0910 巡检已明示「0910 23:59 隐性软截止线已过 = 0911 起 daily 跑出 = 失效日数每日 +1 无任何软截止窗口」+ 0911 23:59 是 T1 决策点 1 在「无下次软截止 + 隐性软截止线也过」状态下的「全无软截止」首日",
            "t4_workflow_impact": "T4 工作流在 B4 5 失效 + 无任何软截止(0910 23:59 已过)环境下仍稳定输出(0906/0908/0909/0910 4 触发 1 未触发 = 巡检后 24h 内 T4 触发 2 commit 节奏稳定),T4 工作流定性升级 = 「硬截止全失效下 T4 工作流自主稳定」——即使 T1 不再承诺 soft deadline,0910 23:59 隐性软截止线也已失效,T4 仍按 0902 模式自举 + 0909 模式双 commit 闭环",
            "verdict": "B4 失效升级(4 → 5)对 T4 工作流无负面影响(0910 仍触发 2 commit);T4 与 T1 解耦彻底 = T4 节奏不依赖 T1 决策 / soft deadline / 隐性 soft deadline / hard deadline / 任何截止线",
        ...
        # 节奏 5(v3.5 沿用,v3.6 无新信号不升级):B4 五失效 + 无任何软截止对 T4 工作流的「零缓冲」首日观察
        "rhythm_b5_impasse": {
            "observation": "0911 是 T1 决策点 1 在「无下次软截止 + 隐性软截止线也过」状态下的「全无软截止」首日(0910 23:59 隐性软截止线已过,0911 巡检明示「本次巡检不再设新的隐性软截止线,因为 0910 23:59 已被用作隐性软截止且已失效,继续设新隐性软截止线 = 实质上降低巡检报告的严肃性」)",
            "t4_workflow_impact": "T4 工作流在「全无软截止」首日(0911)仍按 0902/0909 模式自举(本 cron 0911 v3.5 + 主计划 §5.1 + .plan/20260911.md 闭环)——「全无软截止」对 T4 工作流节奏无任何影响,「无下次软截止」→「无任何软截止」的演化已无新信号,T4 节奏 5 例形成 + 沿用 0909 模式双 commit 3 例形成(0906/0909/0910)稳定不变",
            "verdict": "T4 工作流与 T1 决策点 1 的截止线演化已完全脱钩:从 v3.3「有 soft deadline」(0908 23:59 软截止线) → v3.4「无下次软截止」(0909 23:59 后无下次) → v3.5「无任何软截止」(0910 23:59 隐性软截止线也过),三个阶段 T4 工作流均稳定输出 2 commit / 周期,无节奏衰减",
            "rationale": "v3.3(0909 巡检后 T4 触发 2 commit)/ v3.4(0910 巡检后 T4 触发 2 commit)/ v3.5(0911 巡检后 T4 预期触发 2 commit)= 三阶段恒定 2 commit,「全无软截止」不改变 T4 节奏 → 验证 T4 与 T1 截止线解耦彻底",
        },
        # T4 工作流新阶段定性(v3.6 升级)
        "t4_workflow_stage": "新阶段 = 「巡检后 plan 文档闭环小节奏」(6 例形成:0902/0906/0908/0909/0910/0911) + 「自举入库 + plan 闭环」双 commit 模式(4 例形成:0906/0909/0910/0911)双层稳定节奏;B4 失效 12 次(0912 daily 跑出第 19 次) + 「全无软截止」第 2 日(0912) + 0831 承诺作废信号立第 6 日 + T1 决策模式「集中决策,不分批」连续 11 个工作日(0902-0912)验证 + cron 流续上 13 日 = N=13 等多信号压力下,T4 工作流仍稳定输出 2 commit / 周期,三阶段演化(有 soft deadline / 无下次软截止 / 无任何软截止)均不改变 T4 节奏",
    }
...
def render_md(r: dict) -> str:
    ...
    # T4 工作节奏自审(v3.6: 2026-09-12 快照,"巡检后 plan 文档闭环"小节奏 6 例形成 + T4 即时闭环 6 段观察 + 节奏 4/5 沿用 v3.5)
    if r.get("t4_cadence"):
        c = r["t4_cadence"]
        ...
        # 节奏 1:巡检后 plan 文档闭环小节奏(6 例形成,v3.6 升级)
        rc = c["rhythm_plan_close"]
        lines.append("### 10.1 巡检后 plan 文档闭环小节奏(6 例形成,v3.6 升级)")
        lines.append("")
        ...
        # 节奏 2:T4 即时闭环节奏小结(5 触发 1 未触发,0912 巡检后 T4 触发待观察)
        ri = c["rhythm_t4_immediate"]
        lines.append("### 10.2 T4 即时闭环节奏小结(5 触发 1 未触发,0912 巡检后 T4 触发待观察,v3.6 升级)")
        lines.append("")
        ...
        # 节奏 3:0830 daemon 偶发降级(12 日稳定,v3.6 升级)
        rd = c["rhythm_0830_daemon"]
        lines.append("### 10.3 0830 mavis daemon 单次漏跑正式降级(12 日稳定,v3.6 升级)")
        lines.append("")
        ...
        # 节奏 4(v3.5 沿用,v3.6 无新信号):B4 五失效 + 无任何软截止(0910 23:59 隐性软截止线已过)对 T4 工作流的影响
        rb = c["rhythm_b4_impasse"]
        lines.append("### 10.4 B4 五失效 + 无任何软截止对 T4 工作流的影响(v3.5 升级,v3.6 沿用)")
        lines.append("")
        ...
        # 节奏 5(v3.5 新增,v3.6 沿用):B4 五失效 + 无任何软截止对 T4 工作流的「零缓冲」首日观察
        rb5 = c["rhythm_b5_impasse"]
        lines.append("### 10.5 B4 五失效 + 无任何软截止对 T4 工作流的「零缓冲」首日观察(v3.5 新增,v3.6 沿用)")
        lines.append("")
